...
const Template1 = () => {
  ...
  const saveFormData = async () => {
    try {
        // Assuming serverTimestamp() returns a valid Firestore timestamp
        const timeStamp = serverTimestamp();
        
        // Assuming templateName and user are defined
        const resume_id = `${templateName}-${user?.uid}`;
        
        // Assuming getImage() returns a Promise that resolves to imageURL
        const imageURL = await getImage();
        
        // Assuming loadedTemplateId, formData, education, experiences, skills, and imageAsset are defined
        const _doc = {
        ...
        // Assuming db is a Firestore instance
        await setDoc(doc(db, "users", user?.uid, "resumes", resume_id), _doc);
        
        // Assuming refetch_resumeData() is defined
        refetch_resumeData();
